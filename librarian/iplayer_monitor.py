...
class IPlayerInfoParser(object):
    ...
    '''
    IPLAYER_COMMAND="/home/kevin/get_iplayer/get_iplayer --info --fields pid "
    ...
    def find_pids(self):
        pid_list=[]
        pid_regex=re.compile(".*_([a-z0-9]+)_default\.flv")
        
        for f in glob.glob("/srv/media/IPlayer/*.flv"):
            if pid_regex.match(f):
                pid_list+=pid_regex.match(f).groups
        
        
    def fetch_info(self, query_id):
        return subprocess.check_output(IPLAYER_COMMAND+" "+query_id, shell)
        
    def parse_info(self, info_stream):
        ...
        for line in info_stream:
            ...
            if  ( len(current_show.keys())==(len(fields.keys())+len(optional_fields)))  or blank_re.match(line) and len(current_show.keys())== len(fields.keys()):
                # for iplayer the original filename doesn't include the actual extension. find the file now so we know the real extension:
                real_file_glob=glob.glob(current_show['original']+".*")
                if len(real_file_glob)==1:
                    current_show['original']=real_file_glob[0]
                    #print current_show
    ...
    def parse(self, info_stream):
        file_info_list=self.parse_info(info_stream)
